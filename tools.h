...
namespace tww {
    ...
    node_t test_sequence(Graph& g, vector<pair<node_t, node_t>>& contr) {
        size_t result = 0;
        for(auto& cp: g.red_adjacency)
            cp.reset();
        ...
        for(auto& cc: contr) {
            ...
            if (done.test(cc.first)) {
                cout << "ERROR: Double Merge" << endl;
            }
            done.set(cc.first);

            auto reds = ((g.adjacency[cc.first] ^ g.adjacency[cc.second]) - done) | g.red_adjacency[cc.first];
            reds.reset(cc.second);

            for(auto cn=g.red_adjacency[cc.first].find_first(); cn != g.red_adjacency[cc.first].npos; cn = g.red_adjacency[cc.first].find_next(cn)) {
                g.red_adjacency[cn].reset(cc.first);
            }
